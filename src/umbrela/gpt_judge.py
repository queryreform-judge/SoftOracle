...
class GPTJudge(LLMJudge):
    ...
    def run_batch_requests(self, prompts, max_new_tokens):
        """Run batch requests using OpenAI Batch API."""
        print(f"Processing {len(prompts)} requests in batch mode...")
        
        # OpenAI Batch API supports up to 50,000 requests per batch
        ...
        if len(prompts) > max_batch_size:
            ...
            for i in range(0, len(prompts), max_batch_size):
                ...
                chunk_outputs, chunk_metadata = self._process_batch_chunk_with_retry(
    ...
    def _process_batch_chunk_with_retry(self, prompts, max_new_tokens, offset=0, chunk_num=0, chunk_range=(0, 0), timestamp=None, max_retries=3, retry_interval=10):
        """Wrapper to process a batch chunk with retries on connection/system errors."""
        attempts = 0
        while attempts < max_retries:
            try:
                return self._process_batch_chunk(prompts, max_new_tokens, offset, chunk_num, chunk_range, timestamp)
            except Exception as e:
                attempts += 1
                # Print error
                print(f"Error in batch processing (attempt {attempts}): {e}")
                if attempts < max_retries:
                    print(f"Retrying batch chunk (attempt {attempts+1}/{max_retries}) after {retry_interval}s...")
                    time.sleep(retry_interval)
                else:
                    print(f"All retries failed for batch chunk. Fallback to individual requests.")
                    # Optionally: log error info
                    return [self.run_gpt(prompt, max_new_tokens) for prompt in prompts], {}
    ...
    def _process_batch_chunk(self, prompts, max_new_tokens, offset=0, chunk_num=0, chunk_range=(0, 0), timestamp=None):
        ...
        # Try block is now always handled by _process_batch_chunk_with_retry
        try:
            ...
            poll_interval = random.randint(30, 60)  # Check every 30-60 seconds
            ...
            while time.time() - start_time < max_wait_time:
                ...
                if status == "completed":
                    ...
                    output_content = self.client.files.content(output_file_id).text
                    ...
                    if hasattr(batch_status, "error_file_id") and batch_status.error_file_id:
                        ...
                        try:
                            ...
                            error_content = self.client.files.content(error_file_id).text
            ...
            # Do NOT fallback here! Retry happens in the outer wrapper.
            raise
...
def main():
    ...
    parser.add_argument(
        "--use_batch",
        action="store_true",
        help="Use OpenAI Batch API for processing"
    )
    parser.add_argument(
        "--batch_size",
        type=int,
