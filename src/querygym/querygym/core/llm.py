...
class OpenAICompatibleClient:
    ...
    def __init__(self, model: str, base_url: str | None = None, api_key: str | None = None):
        ...
        Args:
            ...
            api_key: Optional API key. If None, uses OPENAI_API_KEY environment variable.
                     For vLLM servers, this can be None or empty.
        """
        # Get API key from parameter or environment, but handle empty strings
        env_api_key = os.getenv("OPENAI_API_KEY")
        final_api_key = api_key if api_key is not None else env_api_key
        
        # For vLLM and similar servers, don't pass api_key if it's None or empty
        # to avoid "Illegal header value b'Bearer '" error
        client_kwargs = {
    ...
    def chat(self, messages: List[Dict[str, str]], **kw: Any) -> str:
        ...
        Example:
            ...
            ... )
        """
        # Hardcoded OpenRouter provider routing when using OpenRouter API
        kw = dict(kw)
        extra = kw.setdefault("extra_body", {})
        extra["provider"] = {
            "order": ["novita", "hyperbolic"],
            'ignore': ['together'],
