...
class GENQR(BaseReformulator):
    ...
    Modes:
        - "keywords" or "expansion": Uses genqr.keywords.v1 prompt (default)
        - "keywords_variant" or "variant": Uses genqr.keywords.variant.v1 prompt
    ...
    def reformulate(self, q: QueryItem, contexts=None) -> ReformulationResult:
        # Get parameters
        mode = str(self.cfg.params.get("mode", "keywords")).lower()
        ...
        if mode in ["keywords", "expansion", ""]:
            prompt_id = "genqr.keywords.v1"
        elif mode in ["keywords_variant", "variant"]:
            prompt_id = "genqr.keywords.variant.v1"
        else:
            # Fallback to keywords mode
            prompt_id = "genqr.keywords.v1"
